...
// Generate the three default style replies (warm / professional / brief)
// using the shared Western owner-voice prompt. `review` carries reviewer
// name + stars so the prompt can address the guest by first name and
// pick the right tone-by-rating branch.
export async function generateReplies({ openai, profile, review }) {
  const systemPrompt = buildSystemPrompt(profile, "en");
...
export async function ingestReview({ supa, openai, profile, googleReview }) {
  ...
  // Every Places-sourced review name is "places/<PLACE_ID>/reviews/<REVIEW_ID>".
  // If the path's place_id doesn't match the profile's place_id, we are looking
  // at a review for the wrong business — refuse to ingest it.
  const pathMatch = /^places\/([^/]+)\//.exec(googleReviewId);
  const reviewPlaceId = pathMatch ? pathMatch[1] : null;
  if (profile.place_id && reviewPlaceId && reviewPlaceId !== profile.place_id) {
    console.warn(
      `[places-sync] PLACE ID MISMATCH — refusing to ingest. ` +
      `Review path place_id=${reviewPlaceId} but profile.place_id=${profile.place_id}. ` +
      `Review name: ${googleReviewId}`
    );
    return null;
  }
  ...
  try {
    replies = await generateReplies({
      openai,
      profile,
      review: { reviewer_name: reviewerName, stars, content },
    });
  } catch (genErr) {
  ...
  const { data: insertedReview, error: insertErr } = await supa
    ...
    .insert({
      ...
      // Pin the review to the place_id from its own path (preferred), falling
      // back to the profile's current place_id. Used to filter dashboard views
      // when the user reconnects a different business.
      place_id: reviewPlaceId || profile.place_id || null,
    })
    .select()
    .single();
