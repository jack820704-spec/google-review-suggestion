...
function buildPreferenceBlock(prefs = {}, plan = "free_trial") {
  ...
  if (isPro) {
    ...
    if (prefs.length_preference && LENGTH_TARGETS[prefs.length_preference]) {
      ...
      lines.push(`- Target reply length: ${t.label}. (Override the default 50–80 word rule.)`);
    }
  }

  if (lines.length === 0) return "";
  return `\n\nOwner-specific preferences (these OVERRIDE the generic rules above when in conflict):\n${lines.join("\n")}`;
}
